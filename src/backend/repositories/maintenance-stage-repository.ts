...
class MaintenanceStageRepository {
  ...
  async getByTypeAndPlan(
    ...
    try {
      const result = await this.db.query(
        "SELECT get_all_maintenance_stages_by_type_and_plan($1, $2, $3, $4, $5)",
        [maintenanceTypeId, maintenancePlanId, userId, limit, offset]
      );

      const response =
        result.rows[0].get_all_maintenance_stages_by_type_and_plan;
      ...
      console.error("Error al obtener stages por tipo y plan:", err);
      throw err;
    }
  }
  ...
  async getByPlan(
    ...
    try {
      const result = await this.db.query(
        "SELECT get_all_maintenance_stages_by_plan($1, $2, $3, $4)",
        [maintenancePlanId, userId, limit, offset]
      );

      const response = result.rows[0].get_all_maintenance_stages_by_plan;
      ...
      console.error("Error al obtener stages por plan:", err);
      throw err;
    }
  }
